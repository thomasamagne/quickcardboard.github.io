...
function notemodeOn() {
  let currentGuessBtn = document.querySelector(".guess-count[data-state = 'current']")
  let notemodeChecked = document.querySelector("#notemode")
  notemodeChecked.addEventListener('change', function() {
    for (let i = 0; i < tileList.length; i++) {
        console.log('remove active tiles')
    ...
    stopInteraction()
    if (currentGuessBtn.dataset.guesscount == guessCount) {
      startInteraction()
    }
  }
  })
...
function pressKey(key) {
  let notemodeChecked = document.querySelector("#notemode").checked;
  if (!notemodeChecked) {
    ...
    for (let i = 0; i< tileList.length; i++) {
      ...
      if (tileList[i].dataset.state == 'active'){
        ...
        if (!noteList[i].textContent.includes(key)) {
          ...
          noteList[i].textContent = noteList[i].textContent.split('').sort().join('');
        } else {
...
function submitGuess(gc) {
  ...
  if (guessCount > 9 && saveGuess() != answer) {
    alert(
